...
then DeepSeek, then the two Phi-4 Mini variants, then Gemma and CodeLlama.

**What `Label` means:**

| Label | Meaning |
|---|---|
| `RECOMMENDED` | The default, best all-around pick for this kit. |
| `UNCENSORED` | An "abliterated"/uncensored fine-tune with fewer built-in refusals. Use with the same judgment you'd apply to any uncensored model. |
| `STANDARD` | A regular instruction-tuned model with normal safety behavior. |

`Label` is cosmetic (shown in the selection menu so you know what you're
picking) — it doesn't change how the script downloads or runs the model.
`Badge` (shown next to it, e.g. `CODING`, `REASONING`, `WRITING / UI`) is
just a short tag for what the model's good at.

Sizes for entries 2–4 (the newer additions) are estimates based on typical
quantization ratios —
...
the catalog under a different uploader — two duplicates were caught and
removed this way already (a second `Qwen2.5 Coder 7B` and a second
`DeepSeek Coder V2 Lite`, both re-uploads of models already in the list).
